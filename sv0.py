...
#https://books.google.ru/books?id=w86PDwAAQBAJ&pg=PA290&lpg=PA290&dq=cv2.TERM_CRITERIA_EPS&source=bl&ots=q1I9KpZ9ub&sig=ACfU3U28FMeCpeOK2RloE1m-6EBccLxDmQ&hl=ru&sa=X&ved=2ahUKEwiSxKf-g-joAhUCmYsKHfY9C6kQ6AEwBHoECAsQLA#v=onepage&q=cv2.TERM_CRITERIA_EPS&f=false
def nothing(*argv):
        pass
...
def coords_mouse_disp(event,x,y,flags,param): #Function measuring distance to object
    if event == cv2.EVENT_LBUTTONDBLCLK: #double leftclick on disparity map (control windwo)
        ...
        #global
        distanceToObject = str(measure_distance(x,y))
        global distanceToObjectString
        distanceToObjectString = str(str(distanceToObject)+" m")
        ###
...
numDisp = 128  ## global trackbar parameters
bSize = 3
window_size = 3
minDisp = 2
disp12 = 10
...
def updateTrackbar(): #function for reading and adjustring trackbar parameters
    ...
    color = cv2.getTrackbarPos('Color Map', 'control')
    ...
    if(flagObj == 1):
        ...
        for i in range(len(boxesL)):
            if i in indexesL:
                ...
                cv2.circle(sgbm_filteredImg, (centerL[i][0], centerL[i][1]), 10, color,  2)
